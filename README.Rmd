...
The purpose of `cmdr` is to significantly reduce the overhead involved in
...
Briefly, `cmdr` captures R function arguments (**args**) and converts them to
a vector of commandline **flags**. 

The `cmdr` framework provides three
...
```
`cmdr` provides a preexisting utility for this, however. The `cmd_install_check`
...
`cmd_install_check` function, `cmdr` also provides the `cmd_ui_file_exists` function
...
`cmdr` provides additional helper functions to handle common manipulations.
...
`cmdr` additionally provides a few convenience functions for generating lists
...
When using `cmdr` to write lazy shell wrappers, the user can easily mistype a
...
uninformative error messages. `cmdr` has built-in methods to automatically
...
`cmdr` tries to be helpful and identify the misspelling for the user.
```{r, error=T}
