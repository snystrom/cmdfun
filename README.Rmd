...
of expected files. `cmd_file_cmbn` generates combinations of extension/prefix
file names. The output can be passed to `cmd_files_exist` which will error if
a file isn't found on the filesystem.
```{r}
cmd_file_cmbn(ext = c("txt", "xml"), prefix = "outFile")
```

```{r}
cmd_file_cmbn(ext = "txt", prefix = c("outFile", "outFile2", "outFile3"))
```
