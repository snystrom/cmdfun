...
The purpose of `cmdlr` is to significantly reduce the overhead involved in
...
Briefly, `cmdlr` captures R function arguments (**args**) and converts them to
a vector of commandline **flags**. 

The `cmdlr` framework provides three
...
`cmd_crystallize_flags` converts the output of `cmd_args_to_flags` to a vector of
...
This list can be passed to `cmd_crystallize_flags` to generate a vector suitable for `system2` to build shell commands. 
...
`cmd_build_path_handler()` is a macro which returns a function that returns a valid
...
```{r}
handle_meme_path <- cmd_build_path_handler(default_path = "~/meme/bin")
...
```{r}
handle_meme_path <- cmd_build_path_handler(environment_var = "MEME_PATH")
```
...
```
`cmdlr` provides a preexisting utility for this, however. The `cmd_check_install`
...
check_meme_install <- function(path = NULL){
  cmd_check_install(handle_meme_path, path = path)
}
```
...
`cmd_check_install` function, `cmdlr` also provides the `cmd_ui_file_exists` function
...
`cmdlr` also provides a macro `cmd_build_is_valid_install()` to construct
...
function hooks that depend on a command install. `cmd_build_is_valid_install()`
...
```{r}
meme_installed <- cmd_build_is_valid_install(handle_meme_path)
meme_installed()
```

This also works on utils defined during path hanlder construction.
```{r}
ame_installed <- cmd_build_is_valid_install(handle_meme_path, util = "ame")
ame_installed()
```
...
`cmdlr` provides additional helper functions to handle common manipulations.
...
`cmdlr` additionally provides a few convenience functions for generating lists
...
When using `cmdlr` to write lazy shell wrappers, the user can easily mistype a
...
uninformative error messages. `cmdlr` has built-in methods to automatically
...
`cmdlr` tries to be helpful and identify the misspelling for the user.
```{r, error=T}
