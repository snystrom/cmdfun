...
```
`cmdlr` provides a preexisting utility for this, however. The `check_install`
...
`check_install` function, `cmdlr` also provides the `ui_file_exists` function
...
`cmdlr` also provides a macro `build_is_valid_install()` to construct
...
function hooks that depend on a command install. `build_is_valid_install()`
