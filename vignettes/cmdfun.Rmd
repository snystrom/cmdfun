...
flag/value pairs (details below). This output can be useful for additonal
...
Passing flags to commandline software inheritly varies from how arguments are
...
Command executibles can be stored in different locations across devices,
...
provide the location to their install, however, requring this for every function
...
when a user explicity passes a path) before falling back to a less-specific
